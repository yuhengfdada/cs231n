...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        D = np.prod(input_dim)
        self.params['W1'] = np.random.normal(0,weight_scale,(D,hidden_dim))
    ...
    def loss(self, X, y=None):
        ...
        dH1,grads['W2'],grads['b2'] = affine_backward(dloss,cache_scores)
        grads['W2'] += reg * W2
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for H_dim in hidden_dims:
            self.params['W'+str(i)] = np.random.normal(0,weight_scale,(last_dim,H_dim))
            self.params['b'+str(i)] = np.zeros(H_dim)
            last_dim = H_dim
            i += 1
        self.params['W'+str(i)] = np.random.normal(0,weight_scale,(last_dim,num_classes))
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 1: # 输出层没有激活函数
                ...
                Output, cache['Cache' + strid] = affine_batchnorm_relu_forward(Input, Wi, bi, gammai, betai, self.bn_params[i])
        ...
        loss, dloss = softmax_loss(scores, y) # loss函数的正向反向在一起
        dOut = dloss
        for i in range(self.num_layers)[::-1]:
            ...
            if i == self.num_layers - 1: # 输出层没有激活函数
                ...
                if self.normalization=='batchnorm':
                    dX,dW,db,dgamma,dbeta = affine_batchnorm_relu_backward(dOut, cache['Cache'+strid])
...
def affine_batchnorm_relu_backward(dout, cache):
    ...
    daout = relu_backward(dout, relu_cache)
    dbout,dgamma,dbeta = batchnorm_backward_alt(daout, bn_cache)
    dx, dw, db = affine_backward(dbout, fc_cache)
